...
/* WPA3 external supplicant SAE handshake uses entropy function which internally locks mutex. Hence adding
 * mbedtls mutex threading ALT functions
 */

#if defined(MBEDTLS_THREADING_ALT)
__WEAK void cy_mbedtls_mutex_init(mbedtls_threading_mutex_t *mutex)
{
    cy_rtos_init_mutex(&mutex->tls_buf_mutex);
}

__WEAK void cy_mbedtls_mutex_free(mbedtls_threading_mutex_t *mutex)
{
    cy_rtos_deinit_mutex(&mutex->tls_buf_mutex);
}

__WEAK int cy_mbedtls_mutex_lock(mbedtls_threading_mutex_t *mutex)
{
...
__WEAK int cy_mbedtls_mutex_unlock(mbedtls_threading_mutex_t *mutex)
{
...
static int wpa3_crypto_mpi_legendre(const mbedtls_mpi *a, const mbedtls_mpi *p) {
    mbedtls_mpi exp, tmp;
    int res = -2;
    int ret;
...
static cy_rslt_t wpa3_crypto_get_rand_qr_qnr(mbedtls_ecp_group* ecp_grp, uint8_t* qr,
...
static cy_rslt_t wpa3_crypto_get_rand(mbedtls_ecp_group *ecp_grp, mbedtls_mpi *r,
        bool order) {
    mbedtls_mpi tmp;
    mbedtls_ctr_drbg_context ctr_drbg;
    mbedtls_entropy_context entropy;
...
static int wpa3_crypto_point_inverse(const mbedtls_ecp_group *grp,
...
static cy_rslt_t wpa3_crypto_derive_ysqr_from_x(uint8_t *x, uint16_t x_len,
...
static cy_rslt_t wpa3_crypto_derive_y_from_ysqr(mbedtls_ecp_group *ecp_grp,
                                ...
                                mbedtls_mpi *u, uint8_t *xpt, uint8_t *ypt)
{
    ...
    WPA3_EXT_LOG_MSG(("\nWPA3-EXT-SUPP:ECP A\n"));
    WPA3_EXT_HEX_BIGNUM_DUMP((&a));

    WPA3_EXT_LOG_MSG(("\nWPA3-EXT-SUPP:ECP z\n"));
    WPA3_EXT_HEX_BIGNUM_DUMP((&z));

    WPA3_EXT_LOG_MSG(("\nWPA3-EXT-SUPP:ECP za\n"));
    WPA3_EXT_HEX_BIGNUM_DUMP((&za));

    WPA3_EXT_LOG_MSG(("\nWPA3-EXT-SUPP:ECP negb\n"));
    WPA3_EXT_HEX_BIGNUM_DUMP((&negb));

    WPA3_EXT_LOG_MSG(("\nWPA3-EXT-SUPP:ECP P\n"));
    WPA3_EXT_HEX_BIGNUM_DUMP((&(wksp->wpa3_crypto_ctxt->group.P)));

    WPA3_EXT_LOG_MSG(("\nWPA3-EXT-SUPP:ECP N\n"));
    WPA3_EXT_HEX_BIGNUM_DUMP((&(wksp->wpa3_crypto_ctxt->group.N)));
    ...
    WPA3_EXT_HEX_BIGNUM_DUMP((&(negba)));
    ...
    WPA3_EXT_HEX_BIGNUM_DUMP((&negbat));
...
cy_rslt_t wpa3_crypto_start_pwe_generation(wpa3_supplicant_workspace_t *wksp) {
    ...
    WPA3_EXT_HEX_BIGNUM_DUMP((&wksp->wpa3_crypto_ctxt->pwe.X));
    WPA3_EXT_HEX_BIGNUM_DUMP((&wksp->wpa3_crypto_ctxt->pwe.Y));
    WPA3_EXT_HEX_BIGNUM_DUMP((&wksp->wpa3_crypto_ctxt->pwe.Z));
...
cy_rslt_t wpa3_crypto_derive_pwe_from_pt(wpa3_supplicant_workspace_t *wksp)
{
    ...
    WPA3_EXT_HEX_BIGNUM_DUMP((&wksp->wpa3_crypto_ctxt->sta_pt_element.X));
    WPA3_EXT_HEX_BIGNUM_DUMP((&wksp->wpa3_crypto_ctxt->sta_pt_element.Y));
        ...
        uint8_t *ssid, uint8_t *passphrase, uint8_t *output, uint8_t outlen)
{
    ...
    WPA3_EXT_HEX_BIGNUM_DUMP((&pwdval));

    WPA3_EXT_LOG_MSG(("\nWPA3-EXT-SUPP: u1\n"));
    WPA3_EXT_HEX_BIGNUM_DUMP((&u1));
    ...
    WPA3_EXT_HEX_BIGNUM_DUMP((&P1.X));
    WPA3_EXT_HEX_BIGNUM_DUMP((&P1.Y));
    ...
    WPA3_EXT_HEX_BIGNUM_DUMP((&P2.X));
    WPA3_EXT_HEX_BIGNUM_DUMP((&P2.Y));
    ...
    WPA3_EXT_LOG_MSG(("\nWPA3-EXT-SUPP:PT xy point\n"));
    WPA3_EXT_HEX_BIGNUM_DUMP((&PT.X));
    WPA3_EXT_HEX_BIGNUM_DUMP((&PT.Y));
...
cy_rslt_t wpa3_crypto_gen_scalar_and_element(
    ...
    WPA3_EXT_HEX_BIGNUM_DUMP((&crypto_ctxt->sta_commit_element.X));
    WPA3_EXT_HEX_BIGNUM_DUMP((&crypto_ctxt->sta_commit_element.Y));
    WPA3_EXT_HEX_BIGNUM_DUMP((&crypto_ctxt->sta_commit_element.Z));
...
cy_rslt_t wpa3_crypto_chk_own_peer_scalar_element(wpa3_supplicant_workspace_t* workspace)
{
    ...
    if (ret == 1) {
        ...
        if (ret == -1) {
            ...
            WPA3_EXT_HEX_BIGNUM_DUMP((&(workspace->wpa3_crypto_ctxt->sta_scalar)));
            WPA3_EXT_LOG_MSG(("\nWPA3-EXT-SUPP:peer_commit_scalar"));
            WPA3_EXT_HEX_BIGNUM_DUMP((&(workspace->wpa3_sae_context_info.peer_commit_scalar)));
            ...
            if (ret != 0) {
                ...
                WPA3_EXT_HEX_BIGNUM_DUMP((&(workspace->wpa3_crypto_ctxt->sta_commit_element.X)));
                WPA3_EXT_LOG_MSG(("\nWPA3-EXT-SUPP:sta_commit_element Y"));
                WPA3_EXT_HEX_BIGNUM_DUMP((&(workspace->wpa3_crypto_ctxt->sta_commit_element.Y)));
                WPA3_EXT_LOG_MSG(("\nWPA3-EXT-SUPP:peer_commit_element X"));
                WPA3_EXT_HEX_BIGNUM_DUMP((&(workspace->wpa3_sae_context_info.peer_commit_element.X)));
                WPA3_EXT_LOG_MSG(("\nWPA3-EXT-SUPP:peer_commit_element Y"));
                WPA3_EXT_HEX_BIGNUM_DUMP((&(workspace->wpa3_sae_context_info.peer_commit_element.Y)));
...
cy_rslt_t wpa3_crypto_compute_shared_secret(
    ...
    WPA3_EXT_HEX_BIGNUM_DUMP((&ecp_point.X));
